...
from contents.list.reducers import list_reducers
import numpy as np
...
class Store(object):
    ...
    def init_store(self):
        ...
        try:
            for key in reducers:
                new_state[key] = reducers[key](Action('LOAD'))
        except:
            for key in reducers:
                new_state[key] = reducers[key](Action(None))
        self.map_state()
        return new_state
    ...
    def dispatch(self, action):
        ...
        np.save('data.npy', self.data_store)
        self.map_state()


store = Store(list_reducers)
