...
from contents.list.actions import change_listfilter, strikethrough_text
import numpy as np


def get_list_of_products(products, vis_filter):
   prods = filter(lambda x: vis_filter in x['listfilter'], products)
   try:
...
def map_dispatch_to_props(dispatch, widget):
   return {'assign': {
      'item_callback_change': lambda p_id: dispatch(change_listfilter(p_id)),
